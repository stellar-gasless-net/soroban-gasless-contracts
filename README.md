...
* **Implemented and wired for real**: `execute()` now calls `env.current_contract_address().require_auth()` — since this contract implements Soroban's `CustomAccountInterface`, that routes through this contract's own `__check_auth`, which verifies a real WebAuthn/secp256r1 passkey signature (challenge-embedding check, `authenticatorData || SHA-256(clientDataJSON)` reconstruction, `secp256r1_verify` against the stored key). A passkey signature now actually authorizes a transaction, not just verifies in isolation. `verify_passkey_signature()` remains available as a standalone entry point (e.g. for a relayer to pre-check a signature) and shares its core logic with `__check_auth` via one function (`passkey_message_digest`), so there's a single implementation, not two that could drift apart. Covered by 6 tests using a real P-256 keypair, including two that call `__check_auth` directly: accepts a genuine signature over the real payload, and rejects a signature presented for a different payload (replay protection).
* **Not implemented yet**: `auth_contexts` (which calls an authorization actually covers) isn't inspected in `__check_auth` — this wallet doesn't enforce per-call scoping yet, so a valid passkey signature currently authorizes whatever it's attached to. Session keys are still stored via `add_session_key` but not enforced against that scoping either — same underlying gap, now more precisely located.
