...
* **`get_session_key` (added 2026-09-09)**: a read-only getter returning a session key's real registered `SessionData` — until now `add_session_key` could only write this, with no way to read it back, so a dApp or UI had no way to show a user what a session key can actually do without re-deriving it from raw event logs. Returns `None` for a never-registered key. Covered by 2 tests: a registered key returns its exact real data, an unregistered key returns `None`. This is what `gasless-relayer-dashboard`'s Session Keys panel (see that repo's README) reads from — see [Deployment](#deployment) below for a real registered demo session key you can query right now.
...
| | 9 | `NoRecoverySignerSet` | `recover_passkey` called but no recovery signer has been designated |
...
| `account_abstraction_wallet` | `CA5OHHJA6LMSMZNVVBWSTZTPZJWJUZGHDSZSJYX4H5XYEFTUSQEUR45P` |
...
A real demo session key is registered on `account_abstraction_wallet` (`scripts/redeploy_account_abstraction_wallet.sh`'s redeploy on 2026-09-09 added `get_session_key`, then this was registered manually so there's real, non-empty data to query): session key `GBYENETMRAQ67FMIH2YT3MAAMTVO2QEMCSL2UYXHD5V45P35LNOZEKK6` may call `transfer` on the native XLM SAC, capped at 50 XLM cumulative. Query it yourself:
```bash
stellar contract invoke --id CA5OHHJA6LMSMZNVVBWSTZTPZJWJUZGHDSZSJYX4H5XYEFTUSQEUR45P --source deployer --network testnet -- get_session_key --session_key GBYENETMRAQ67FMIH2YT3MAAMTVO2QEMCSL2UYXHD5V45P35LNOZEKK6
```
...
| Repository | Tech Stack | Primary Responsibilities | Live Demo | Governance Guide |
| :--- | :--- | :--- | :--- | :--- |
| **[`soroban-gasless-contracts`](https://github.com/stellar-gasless-net/soroban-gasless-contracts)** | Rust, Soroban SDK v21 | WASM Smart Contracts: `trusted-forwarder`, `token-paymaster`, `voucher-paymaster`, `account-abstraction-wallet`. | — (no frontend; see [`deployments/testnet.json`](deployments/testnet.json) + [stellar.expert](https://stellar.expert/explorer/testnet)) | [Contracts Guide](./CONTRIBUTING.md) |
| **[`stellar-gasless-relayer`](https://github.com/stellar-gasless-net/stellar-gasless-relayer)** | TypeScript, Node.js, Express | Backend Relayer Engine: Multi-keypair queue pool, Horizon RPC simulation, FeeBump builder, Prometheus telemetry. | — (backend service; the dashboard below is its UI) | [Relayer Guide](https://github.com/stellar-gasless-net/stellar-gasless-relayer/blob/main/CONTRIBUTING.md) |
| **[`stellar-gasless-sdk`](https://github.com/stellar-gasless-net/stellar-gasless-sdk)** | TypeScript, WebAuthn, React | Client Library & Wallet Adapters: `GaslessClient`, WebAuthn Passkey biometric signer, Freighter / xBull adapters. | — (library, no standalone UI) | [SDK Guide](https://github.com/stellar-gasless-net/stellar-gasless-sdk/blob/main/CONTRIBUTING.md) |
| **[`gasless-relayer-dashboard`](https://github.com/stellar-gasless-net/gasless-relayer-dashboard)** | Vanilla JS, HTML5, CSS3 | Protocol Console: Paymaster XLM deposits, dApp API key portal, live Horizon RPC broadcast simulator. | **[gasless-relayer-dashboard.vercel.app](https://gasless-relayer-dashboard.vercel.app/)** | [Console Guide](https://github.com/stellar-gasless-net/gasless-relayer-dashboard/blob/main/CONTRIBUTING.md) |
